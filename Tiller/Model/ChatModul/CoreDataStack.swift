...
class CoreDataStack {
    ...
    //fetch message From Coredata file
    func getLastMsgImage(senderId: String, receiverId: String) -> [[String: Any]]  {
        var resultArray = [[String: Any]]()
        
        if #available(iOS 10.0, *) {
            let fetchRequest = NSFetchRequest<ChatData>(entityName: "ChatData")
            if receiverId != "" {
                let predicate = NSPredicate(format: "(senderId == %@ AND receiverId == %@ AND type = %@) OR (senderId == %@ AND receiverId == %@ AND type = %@)",senderId,receiverId,"incoming",receiverId, senderId,"incoming")
                fetchRequest.predicate = predicate
            }
    ...
    func delete(senderId: String, receiverId: String){
        ...
        if #available(iOS 10.0, *) {
            ...
            if (!result.isEmpty) {
                print(result)
                for res in result {
                    self.persistentContainer.viewContext.delete(res)
                }
            }
            (UIApplication.shared.delegate as! AppDelegate).coreDataStack.saveContext()
        } else {
            ...
            if (!result.isEmpty) {
                ...
                if (!result.isEmpty) {
                    print(result)
                    for res in result {
                         self.managedObjectContext.delete(res)
                    }
                }
            }
            (UIApplication.shared.delegate as! AppDelegate).coreDataStack.saveContext()
